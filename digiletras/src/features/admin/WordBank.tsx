...
function exportCSV(list: Word[]) {
  ...
  a.href = url; a.download = 'digiletras-palavras.csv'; a.click();
  ...
  return (
    <div className="p-4">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-2xl font-bold text-gray-800">📚 Banco de Palavras</h2>
        <div className="flex gap-2">
          <button
            onClick={() => exportCSV(filtered)}
            className="px-3 py-1.5 bg-green-100 text-green-800 rounded-xl font-bold text-xs"
          ...
          <button
            ...
            className="px-3 py-1.5 bg-blue-100 text-blue-800 rounded-xl font-bold text-xs"
      ...
      {importMsg && (
        <div className={`rounded-xl p-2 text-xs mb-3 ${importMsg.startsWith('✅') ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
          {importMsg}
        </div>
      )}
          ...
          <div key={word.id} className="bg-white rounded-xl p-3 shadow flex items-center gap-2">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1">
                ...
                <button onClick={() => speak(word.word)} className="text-gray-400 hover:text-purple-500 text-xs flex-shrink-0" aria-label="Ouvir">🔊</button>
              </div>

...
const GAME_TYPES: { id: TrackGameType; label: string; emoji: string }[] = [
  { id: 'quiz',          label: 'Quiz',           emoji: '❓' },
  { id: 'memory',        label: 'Memória',        emoji: '🧠' },
  { id: 'syllable',      label: 'Sílabas',        emoji: '🔤' },
  { id: 'fill',          label: 'Completar',       emoji: '✏️' },
  { id: 'write',         label: 'Escrever',        emoji: '📝' },
  { id: 'firstletter',   label: 'Letra Inicial',   emoji: '🅰️' },
  { id: 'buildsentence', label: 'Montar Frase',    emoji: '📖' },
  { id: 'story',         label: 'História',        emoji: '📚' },
  { id: 'matchgame',     label: 'Ligar/Contar',    emoji: '🔗' },
];
              ...
              <div key={track.id} className="ds-card" style={{
                ...
                <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                  <button onClick={() => handleDuplicate(track)}
                    className="ds-btn ds-btn-icon" style={{ background: '#E8F5E9', color: '#00B894', width: 34, height: 34, fontSize: 14 }} title="Duplicar" aria-label="Duplicar trilha">📋</button>
                  {!track.builtin && (
                    <>
                      <button onClick={() => setEditing(track)}
                        className="ds-btn ds-btn-icon" style={{ background: '#EDE7F6', color: '#6C5CE7', width: 34, height: 34, fontSize: 14 }} title="Editar" aria-label="Editar trilha">✏️</button>
                      <button onClick={() => handleDelete(track.id)}
                        className="ds-btn ds-btn-icon" style={{ background: '#FFEBEE', color: '#E53935', width: 34, height: 34, fontSize: 14 }} title="Excluir" aria-label="Excluir trilha">🗑️</button>
                    </>
                  )}
                </div>
              </div>
                  ...
                  <div style={{
                    ...
                    {activities.map((act, ai) => {
                      ...
                      return (
                        ...
                        >
                          <span style={{ fontSize: 14 }}>{gt?.emoji ?? '🎮'}</span>
                          <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              {act.gameType === 'matchgame'
                                ...
                                : act.gameType === 'story'
                                  ? act.storyId
                                    ...
                                    : '⚠️ Nenhuma história selecionada'
                    ...
                    return (
                      <button key={g.id} onClick={() => setMatchGameId(g.id)} style={{
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: 10, color: 'var(--color-text-2)' }}>
                            {g.mode === 'connect' ? '🔗 Ligar' : g.mode === 'count' ? '🔢 Contar' : g.mode === 'type' ? '⌨️ Digitar' : '🔤 Alfabeto'} · {g.pairs.length} pares · Nível {g.difficulty}
                          </div>
                        </div>
                        {selected && <span style={{ color: '#6C5CE7', fontWeight: 800 }}>✓</span>}
                      </button>

...
const STORAGE_KEY = 'digiletras_custom_activities';
...
/** Human-readable label for each game type */
export const GAME_TYPE_LABELS: Record<CustomGameType, { label: string; emoji: string }> = {
  syllable:      { label: 'Sílabas',         emoji: '🧩' },
  quiz:          { label: 'Quiz Visual',      emoji: '🖼️' },
  fill:          { label: 'Completar',        emoji: '✏️' },
  memory:        { label: 'Memória',          emoji: '🧠' },
  write:         { label: 'Escrever',         emoji: '✍️' },
  firstletter:   { label: 'Letra Inicial',    emoji: '🔤' },
  buildsentence: { label: 'Montar Frase',     emoji: '📝' },
  matchgame:     { label: 'Ligar / Digitar',  emoji: '🔗' },
  coloring:      { label: 'Pintar',           emoji: '🎨' },
  story:         { label: 'Histórias',        emoji: '📖' },
};

  ...
  return (
    <div className="min-h-screen p-4 flex flex-col" style={{ background: 'linear-gradient(135deg, #e3f2fd 0%, #667eea 100%)' }}>
      ...
      <div className="flex items-center gap-2 mb-3">
        <button onClick={onBack} className="w-9 h-9 flex items-center justify-center rounded-full bg-white/30 hover:bg-white/50 transition-colors cursor-pointer" aria-label="Voltar"><ArrowLeft size={18} className="text-blue-900" /></button>
      ...
      {mode === 'dictation' && (
        <div className="flex gap-2 mb-3">
          ...
          >
            <Volume2 size={16} className="inline mr-1" /> Ouvir
          </button>
          <button
            onClick={() => setRevealed(r => !r)}
            className="px-4 py-2 bg-purple-500 text-white rounded-xl font-bold"
          >
            <Eye size={16} className="inline mr-1" /> Espiar
          </button>
        </div>
      )}

...
import { ClipboardList, Pencil, Trash2, Save, ArrowLeft, Star } from 'lucide-react';
    ...
    <div style={{ padding: 16 }}>
            ...
            <div key={act.id} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--color-text-2)', marginTop: 2 }}>
                  <LucideIcon name={gameInfo.icon} size={12} style={{ display: 'inline', verticalAlign: 'middle', marginRight: 4 }} />{gameInfo.label} · N{act.difficulty}
                </div>
      ...
      {filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: 48, color: 'var(--color-text-3)' }}>
          <div style={{ fontSize: 48, marginBottom: 8 }}><ClipboardList size={48} style={{ margin: '0 auto' }} /></div>
...
function ActivityForm({ activity, onSave, onCancel }: FormProps) {
  ...
  const gameTypes = Object.entries(GAME_TYPE_LABELS) as [CustomGameType, { label: string; icon: string }][];

  return (
    <div style={{ padding: 16, overflowY: 'auto', maxHeight: '100vh' }}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
        <button onClick={onCancel} style={{ background: 'none', border: 'none', fontSize: 22, cursor: 'pointer', color: 'var(--color-primary)', display: 'flex', alignItems: 'center' }}><ArrowLeft size={22} /></button>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginTop: 4 }}>
          {gameTypes.map(([id, info]) => (
            ...
            >
              <span style={{ fontSize: 22 }}><LucideIcon name={info.icon} size={22} /></span>
              {info.label}
            </button>
          ))}
        </div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
          {([1, 2, 3] as const).map(d => (
            ...
            >
              {d === 1 ? <><Star size={14} className="inline" /> Fácil</> : d === 2 ? <><Star size={14} className="inline" /><Star size={14} className="inline" /> Médio</> : <><Star size={14} className="inline" /><Star size={14} className="inline" /><Star size={14} className="inline" /> Difícil</>}
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <button onClick={handleSave} style={{
          ...
          <Save size={16} style={{ display: 'inline', verticalAlign: 'middle', marginRight: 4 }} />Salvar
        </button>
      </div>

...
import { Pencil, Trash2, Save, ArrowLeft, Palette, FolderOpen, Upload, Eye, EyeOff } from 'lucide-react';
import {
  ...
  return (
    <div className="p-4">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2"><Palette size={20} /> Pintar</h2>
        <button
          ...
          <div key={sheet.id} className="bg-white rounded-2xl p-4 shadow flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-400">
                {isAssetSheet(sheet.id) ? <><FolderOpen size={10} className="inline" /> Arquivo</> : <><Upload size={10} className="inline" /> Upload</>}
              </p>
            </div>
            {/* Small SVG preview */}
            {sheet.svgContent && (
            ...
            {isAssetSheet(sheet.id) ? (
              <button
                ...
                className="px-3 py-1.5 bg-gray-100 text-gray-500 rounded-xl text-xs font-bold flex items-center gap-1"
      ...
      {sheets.length === 0 && (
        <div className="text-center py-8 text-gray-400">
          <p className="text-4xl mb-2"><Palette size={48} className="mx-auto" /></p>
  ...
  return (
    <div className="p-4">
      <div className="flex items-center gap-3 mb-4">
        <button onClick={onCancel} className="text-orange-700 text-xl font-bold"><ArrowLeft size={20} /></button>
      ...
      <label className="block w-full py-4 border-2 border-dashed border-orange-300 rounded-2xl text-center cursor-pointer hover:bg-orange-50 mb-3">
        <span className="text-3xl block mb-1"><FolderOpen size={32} className="mx-auto" /></span>
      ...
      <div className="flex gap-2">
        ...
        <button onClick={handleSave} className="flex-1 py-3 bg-orange-500 text-white rounded-2xl font-bold">
          <Save size={16} className="inline" /> Salvar
        </button>
      </div>
    </div>
  );

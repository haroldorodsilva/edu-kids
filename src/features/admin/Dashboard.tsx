...
export default function Dashboard() {
  ...
  return (
    <div className="p-4">
      <h2 className="text-2xl font-bold text-gray-800 mb-4 flex items-center gap-2"><BarChart3 size={22} /> Dashboard</h2>

      {/* Banco de dados */}
      <div className="grid grid-cols-2 gap-3 mb-5">
        {[
          { label: 'Palavras no banco', value: String(words.length), icon: 'BookOpen' },
          { label: 'Histórias', value: String(stories.length + customCount), icon: 'BookText' },
          { label: 'Jogos disponíveis', value: '8', icon: 'Gamepad2' },
          { label: 'Histórias criadas', value: String(customCount), icon: 'Sparkles' },
        ].map((stat, i) => (
          <div key={i} className="bg-white rounded-2xl p-3 shadow text-center">
            <div className="text-2xl mb-1 flex justify-center"><LucideIcon name={stat.icon} size={24} /></div>
      ...
      {/* Sessão atual */}
      <h3 className="font-bold text-gray-700 mb-3 flex items-center gap-2"><Clock size={16} /> Sessão atual</h3>
      {totalGamesPlayed === 0 ? (
      ...
      {topErrorWords.length > 0 && (
        <div className="bg-white rounded-2xl p-4 shadow mb-4">
          <h4 className="font-bold text-gray-600 text-sm mb-2 flex items-center gap-1"><AlertTriangle size={14} /> Palavras com mais erros</h4>
